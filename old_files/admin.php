...
try {
    ...
    $sql_rides = "SELECT DATE(start_date) as date, COUNT(*) as count FROM ride GROUP BY DATE(start_date)";
    $stmt_rides = $pdo->prepare($sql_rides);
    $stmt_rides->execute();
    $rides_data = $stmt_rides->fetchAll(PDO::FETCH_ASSOC);

    // Crédits gagnés par jour (2 crédits par covoiturage terminé)
    $sql_credits = "
        SELECT DATE(start_date) as date, COUNT(*) * 2 as total 
        FROM ride 
        WHERE status = 'terminé' 
        GROUP BY DATE(start_date)";
    $stmt_credits = $pdo->prepare($sql_credits);
    $stmt_credits->execute();
    $credits_data = $stmt_credits->fetchAll(PDO::FETCH_ASSOC);
    
    // Total des crédits gagnés par la plateforme
    $sql_total_credits = "SELECT COUNT(*) * 2 as total FROM ride WHERE status = 'terminé'";
    $stmt_total = $pdo->prepare($sql_total_credits);
    $stmt_total->execute();
    $total_credits = $stmt_total->fetchColumn();
...
<header>
    ...
    <p><?php echo htmlspecialchars($total_credits); ?> crédits</p>
    ...
    <script>
        const ridesData = <?php echo json_encode($rides_data); ?>;
        const creditsData = <?php echo json_encode($credits_data); ?>;
