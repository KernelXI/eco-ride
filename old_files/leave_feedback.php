...
$ride_id = isset($_GET['ride_id']) ? intval($_GET['ride_id']) : null; // Récupérer l'ID du covoiturage

// Initialiser les variables pour le message et la note
$message = '';
$note = '';
$status = 'pending';

// Vérifier si l'utilisateur a déjà laissé un avis pour ce covoiturage
if ($ride_id) {
    try {
        ...
        // Si un avis existe déjà, récupérer les données
        if ($feedback) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $status === 'pending') {
    ...
    try {
        ...
        // Rediriger vers l'historique des covoiturages après l'enregistrement
